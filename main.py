...
# Юзернейм администратора для рассылки (без знака @)
    ...
    if os.path.exists(DB_FILE) and os.path.getsize(DB_FILE) > 0:
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                ...
                # Принудительно переводим ключи в int, так как JSON сохраняет их как str
...
waiting_for_broadcast = set()  # Состояние ожидания текста рассылки
...
def get_main_keyboard(message_or_username):
    ...
    if username == ADMIN_USERNAME:
        ...
        markup.add('📢 Рассылка всем')  # Кнопка доступна ТОЛЬКО админу
    else:
        markup.add('🔍 Начать поиск', '👥 Групповой чат')
    return markup
...
# Хэндлер кнопки рассылки (только для админа)
@bot.message_handler(func=lambda message: message.text == '📢 Рассылка всем')
def broadcast_command(message):
    user_id = message.chat.id
    username = message.from_user.username
    if username != ADMIN_USERNAME:
        return # Обычные юзеры игнорируются
        
    waiting_for_broadcast.add(user_id)
    bot.send_message(user_id, "📝 Напишите текст сообщения или отправьте любой медиафайл (картинку/стикер), который нужно разослать ВСЕМ пользователям бота:", reply_markup=get_cancel_keyboard())
...
def chat(message):
    ...
    if user_id in waiting_for_broadcast:
        ...
        for u_id in all_users:
            try:
                ...
                success_count += 1
                time.sleep(0.05) # Защита от спам-лимитов Telegram
            except Exception:
                pass # Пропускаем тех, кто заблокировал бота
                
        bot.send_message(user_id, f"✅ Рассылка успешно завершена!\nДоставлено: {success_count} пользователям.")
        return
    ...
    if user_id in waiting_for_photo:
        if message.content_type == 'photo':
            ...
            save_data()
            waiting_for_photo.discard(user_id)
            bot.send_message(user_id, "✅ Картинка профиля успешно установлена!", reply_markup=get_main_keyboard(username))
            show_profile_card(user_id, user_id)
        else:
    ...
    if user_id in active_chats:
        ...
        for member in groups.get(gid, []):
            if member != user_id:
                if message.content_type == 'text': 
                    bot.send_message(member, f"{user_emoji} {name}: {message.text}")
                else: 

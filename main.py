...
# ==================== НАСТРОЙКИ БОТА ====================
API_TOKEN = "СЮДА_ВСТАВЬ_ТОКЕН_ОТ_BOTFATHER"  # Замени на токен своего бота
ADMIN_USERNAME = "VibeNixS"                    # Твой юзернейм (без @)
RENDER_URL = None                              # Ссылка для само-пинга (если деплоишь на Render.com)
# ========================================================
...
# Быстрая админ-панель для кнопок под сообщениями
...
# --- ВЫЗОВ АДМИН-ПАНЕЛИ ЧЕРЕЗ REPLY-КОМАНДЫ ---
...
def admin_action_commands(message):
    ...
    if not message.reply_to_message:
        return bot.reply_to(message, "⚠️ Ответь этой командой на сообщение юзера, чтобы вызвать кнопки модерации.")
    ...
    bot.send_message(
        message.chat.id,
        f"🛠 **Панель управления пользователем**\n"
        f"👤 Юзер: {target_name} (ID: `{target_id}`)\n"
        f"👑 Панель открыл: @{ADMIN_USERNAME}",
        parse_mode="Markdown",
        reply_markup=get_action_keyboard(target_id)
    )

# --- ОБРАБОТКА ИНЛАЙН КНОПОК БЫСТРОЙ АДМИН-ПАНЕЛИ ---
...
def process_panel_callback(call):
    if call.from_user.username != ADMIN_USERNAME:
        return bot.answer_callback_query(call.id, "❌ У тебя нет прав!", show_alert=True)
    ...
    if action == "mute":
        ...
        bot.edit_message_text(
            ...
            text=f"🤐 Пользователь `[ID: {target_id}]` отправлен в **мут**.\n👑 Модератор: @{ADMIN_USERNAME}",
            parse_mode="Markdown"
        )
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"🔊 Пользователь `[ID: {target_id}]` успешно **размучен**.\n👑 Модератор: @{ADMIN_USERNAME}",
            parse_mode="Markdown"
        )
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"🚫 Пользователь `[ID: {target_id}]` отправлен в **бан**.\n👑 Модератор: @{ADMIN_USERNAME}",
            parse_mode="Markdown"
        )
        try: bot.send_message(target_id, "🚫 Вы заблокированы в боте.") 
        except Exception: pass
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            text=f"✅ Пользователь `[ID: {target_id}]` успешно **разбанен**.\n👑 Модератор: @{ADMIN_USERNAME}",
            parse_mode="Markdown"
        )
        try: bot.send_message(target_id, "✅ Ваш профиль разблокирован администратором.") 
        except Exception: pass
...
def broadcast_command(message):
    ...
    bot.send_message(user_id, "📝 Напишите текст сообщения или отправьте медиафайл для рассылки ВСЕМ:", reply_markup=get_cancel_keyboard(user_id))
...
def report_handler(message):
    ...
    if ADMIN_ID:
        ...
        bot.send_message(ADMIN_ID, f"🚨 Новая жалоба на пользователя {get_display_name(target_id)} (ID: `{target_id}`). Всего жалоб: {len(pending_reports[target_id])}", reply_markup=markup, parse_mode="Markdown")
...
def admin_execute_verdict(call):
    ...
    if action == "warn":
        ...
        bot.send_message(call.message.chat.id, f"✅ Выдано предупреждение. Всего жалоб: {user_data[target_id]['reports']}/5")
        try: bot.send_message(target_id, "⚠️ Администратор выдал тебе предупреждение!")
        except Exception: pass
        
    elif action == "mute":
        user_data[target_id]["muted"] = True
        bot.send_message(call.message.chat.id, f"🔇 Ограничение отправки сообщений (Мут) включено.")
        try: bot.send_message(target_id, "🔇 Ты получил мут от администратора.")
        except Exception: pass
        
    elif action == "ban":
        user_data[target_id]["banned"] = True
        bot.send_message(call.message.chat.id, f"🚫 Юзер полностью забанен.")

...
def ask_name(message):
    msg = bot.send_message(message.chat.id, "✍️ Напиши имя, которое будет отображаться в чатах:", reply_markup=get_cancel_markup())
    bot.register_next_step_handler(msg, save_name)

def save_name(message):
    user_id = message.chat.id
    # Если пользователь вместо имени нажал "Отмена"
    if message.text == '❌ Отмена':
        bot.send_message(user_id, "🚫 Ввод имени отменен.", reply_markup=get_start_markup())
        return

# Логика смены кнопок при выборе Поиска или Группы
@bot.message_handler(func=lambda m: m.text in ['🔍 Начать поиск', '👥 Групповой чат'])
def action_selected(message):
    bot.send_message(message.chat.id, f"Вы выбрали: {message.text}. Для возврата нажмите отмену.", reply_markup=cancel_menu())

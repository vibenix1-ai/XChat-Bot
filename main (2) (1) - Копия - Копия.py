...
waiting_for_admin_search = set() # Состояние ожидания ID в админ-панели
...
def get_profile_keyboard(user_id, viewer_username=None):
    ...
    # Если зашел сам админ VibeNixS в свой СОБСТВЕННЫЙ профиль
    if viewer_username == ADMIN_USERNAME and user_id == bot.get_me().id:  # Или сверка по сессии
        pass 

    # Проверка: если это чужой профиль и смотрит админ
    if viewer_username == ADMIN_USERNAME and user_id != user_data:
        # Панель управления над пользователем
    ...
    # Дефолтные кнопки для обычных пользователей (или для админа в своем профиле)
    ...
    # Дополнительная кнопка входа в Админ-Панель, если это профиль самого @VibeNixS
...
def show_profile_card(chat_id, target_user_id, send_share_btn=False, viewer_username=None):
    ...
    # Корректный статус ограничений: по умолчанию "Нет ⚫", при наличии "Есть 🔴"
    ...
    # Передача контекста админа для генерации правильных кнопок
    if chat_id == target_user_id:
        markup = get_profile_keyboard(target_user_id, viewer_username=viewer_username)
...
def ask_name_callback(call):
    ...
    waiting_for_name.add(user_id)
    bot.answer_callback_query(call.id)
    bot.send_message(user_id, "✍️ Напиши имя:")
...
def stop(message):
    ...
    if user_id in waiting_for_name or user_id in waiting_for_emoji or user_id in waiting_for_photo or user_id in waiting_for_broadcast or user_id in waiting_for_admin_search:
        waiting_for_name.discard(user_id)
        waiting_for_emoji.discard(user_id)
        waiting_for_photo.discard(user_id)
        waiting_for_broadcast.discard(user_id)

...
# Глобальный пре-хэндлер для блокировки кликов по кнопкам, если юзер в муте
@bot.callback_query_handler(func=lambda call: user_data.get(call.message.chat.id, {}).get("is_muted", False))
def block_buttons_in_mute(call):
    bot.answer_callback_query(call.id, "🔇 Вы замучены! Кнопки заблокированы.", show_alert=True)
...
def report_init_handler(message):
    ...
    bot.send_message(user_id, "📋 Пожалуйста, опишите причину жалобы на собеседника:\n*(После отправки жалобы чат будет автоматически завершен!)*", parse_mode="Markdown", reply_markup=get_report_cancel_keyboard())
...
def chat(message):
    ...
    # Обработка написания причины жалобы + МГНОВЕННЫЙ РАЗРЫВ ЧАТА
    if user_id in waiting_for_report_reason:
        waiting_for_report_reason.discard(user_id)
        ...
        # 1. Сначала определяем нарушителя
        if user_id in active_chats:
            target_id = active_chats[user_id]
        elif user_id in user_to_group:
            is_group_report = True
            saved_gid = user_to_group[user_id]
            group_members = [m for m in groups.get(saved_gid, []) if m != user_id]
            if group_members:
                target_id = group_members[-1] 
                
        if not target_id:
            bot.send_message(user_id, "❌ Не удалось определить нарушителя. Возможно, чат уже был закрыт.", reply_markup=get_main_keyboard(username))
            return

        # 2. Сохраняем логи для отправки админу
        logs_text = "⚠️ Сообщений в логе нет."
        if user_id in chat_logs and chat_logs[user_id]:
            logs_text = "\n".join(chat_logs[user_id])

        # 3. Мгновенно разрываем соединение чата для обоих участников
        if not is_group_report:
            # Тет-а-тет чат
            active_chats.pop(user_id, None)
            active_chats.pop(target_id, None)
            chat_logs.pop(user_id, None)
            chat_logs.pop(target_id, None)
            
            bot.send_message(user_id, "🚨 Чат экстренно завершен из-за подачи жалобы.", reply_markup=get_main_keyboard(username))
            bot.send_message(target_id, "🚫 Диалог завершен. На вас была отправлена жалоба администратору.", reply_markup=get_main_keyboard(user_data.get(target_id, {}).get("username")))
            send_rating(user_id)
            send_rating(target_id)
        else:
            # Групповой чат
            if saved_gid in groups:
                groups[saved_gid].remove(user_id)
                user_to_group.pop(user_id, None)
                chat_logs.pop(user_id, None)
                
                bot.send_message(user_id, "🚨 Вы покинули группу и отправили жалобу.", reply_markup=get_main_keyboard(username))
                for member in groups[saved_gid]: 
                    bot.send_message(member, f"👋 {name} покинул группу (Подана жалоба).")
                if not groups[saved_gid]: 
                    del groups[saved_gid]

        # 4. Пересылаем жалобу администратору (@VibeNixS)

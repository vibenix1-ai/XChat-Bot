...
RENDER_URL = os.environ.get('RENDER_EXTERNAL_URL') # Твоя ссылка на Render для само-пинга
...
# Поток для предотвращения отключения Render (Anti-Sleep)
def keep_alive():
    if not RENDER_URL:
...
def get_main_keyboard():
    # Обычные текстовые кнопки чатов внизу экрана
...
def get_profile_keyboard(user_id):
    ...
    # Динамически скрываем/показываем кнопки в зависимости от рефералов
    if refs >= 5:

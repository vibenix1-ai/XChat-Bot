...
def stop(message):
    ...
    if user_id in waiting_users:
        ...
        bot.send_message(user_id, "⚠️ Вы не в поиске/чате.", reply_markup=get_main_keyboard())
...
def name_command(message):
    ...
    # Проверяем, не находится ли пользователь в чате или поиске
    if user_id in active_chats or user_id in user_to_group or user_id in waiting_users:
        bot.send_message(user_id, "⚠️ Нельзя менять имя во время поиска или активного чата.")
        return

    # Включаем режим ожидания имени
    waiting_for_name.add(user_id)
    
    # Меняем нижнюю клавиатуру на "Отмена", чтобы пользователь мог выйти из этого режима
    bot.send_message(
        user_id, 
